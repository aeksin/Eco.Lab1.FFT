...
void measureEcoFFT(uint32_t size, IEcoVectorPtr_t pIEcoVector, IEcoLab1Ptr_t pIEcoLab1)
{
    ...
    if (size == 8)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            result = pIEcoVector->pVTbl->Get(SourceVector, i, &tmp, type_complex);
            printf("%lf %lf\n", tmp.real, tmp.imag);
        }
...
Release2:
    if (SourceVector != 0)
    {
        SourceVector->pVTbl->Release(SourceVector);
    }
    if (ResultVector != 0)
    {
        ResultVector->pVTbl->Release(ResultVector);
    }
}
...
int16_t EcoMain(IEcoUnknown *pIUnk)
{
    ...
    uint32_t size = 1;
    uint32_t max_size = 1 << 27;
    uint32_t max_check_size = 1 << 16;
...
Release:
    ...
    if (pIEcoVector != 0)
    {
        pIEcoVector->pVTbl->Release(pIEcoVector);
    }
